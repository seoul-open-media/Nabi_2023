...
void getSerialDataSendToHost() {
  ...
  if (Serial.available() > 6) {
    ...
    if (first_byte == 255) {
      ...
      // send out rightaway
      byte d_data[19];
      d_data[0] = Serial.read();
      d_data[1] = Serial.read();
      d_data[2] = Serial.read();
      d_data[3] = Serial.read();
      d_data[4] = Serial.read();
      d_data[5] = Serial.read();
      d_data[16] = 100; // to escape ranging confusion
      d_data[17] = 100; // host computer address
      d_data[18] = my_address; // my address
      
      DW1000Ng::setTransmitData(d_data, 19);
      DW1000Ng::startTransmit();
...
void loop() {
  ...
  if (my_mode == initiator_mode)
  {
    twrInitiator();
  }
  else if (my_mode == responder_mode){
    twrResponder();
  }
  getSerialDataSendToHost();
